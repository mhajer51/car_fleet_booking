...
<head>
    <meta charset="UTF-8">
    <title>{{ __('Booking updated by user') }}</title>
    <style>
        ...
        .note { margin-top: 16px; padding: 12px; border-left: 3px solid #16a34a; background: #f0fdf4; }
    </style>
</head>
<body>
<div class="card">
    <h1 style="margin-top: 0;">{{ __('A submitted booking was modified') }}</h1>
    <p class="muted">{{ __('The requester updated an existing reservation. Review the latest details below to keep approvals in sync.') }}</p>
    ...
    <table class="details">
        ...
        <tr>
            ...
            <td>{{ optional($booking->updated_at)->timezone(config('app.timezone'))->format('d M Y H:i') }}</td>
        </tr>
    </table>

    @if ($booking->note)
        <div class="note">
            <strong>{{ __('Requester notes') }}:</strong>
